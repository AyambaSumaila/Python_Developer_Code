...
for event in events:
    ...
    display='It is %s days untill %s' % (days_until, event_name)
...
def get_events():
    ...
    with open('events.txt') as file: 
        for line in file: 
            ...
            current_date=datetime.striptime(current_event[1], '%d/%d/%y').date()
            list_events.append(current_event)
            
    return list_events 


def days_between_dates(date, date2):
    time_between=str(date - date2)
